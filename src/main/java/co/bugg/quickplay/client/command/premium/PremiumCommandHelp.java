    ...
    public void run(String[] args) {
        ...
        if(args.length < 2) {
            ...
            if(filteredList.size() > 0) {
                // Send information on how to use the specified command, i.e. syntax
                final IChatComponent chatComponent = new ChatComponentText(filteredList.get(0).getHelpText() + "\n")
                        ...
                        .appendSibling(new ChatComponentText("/" + parent.getParent().getCommandName() + " " +
                                parent.getName() + " " + filteredList.get(0).getName() + " " + filteredList.get(0).getUsage()))
    ...
    public void sendBaseHelpMessage() {
        ...
        if(parent.premiumCommands.size() > 0) {
            ...
            for(ListIterator<IPremiumCommand> iter = parent.premiumCommands.listIterator(); iter.hasNext();) {
                ...
                message.appendSibling(new ChatComponentText("/" + parent.getParent().getCommandName() + " " +
                        parent.getName() + " " + premiumCommand.getName()))
                    .setChatStyle(new ChatStyle().setColor(EnumChatFormatting.AQUA));

                message.appendSibling(new ChatComponentText(" - ")
                        .setChatStyle(new ChatStyle().setColor(EnumChatFormatting.GRAY)));

                message.appendSibling(new ChatComponentText(premiumCommand.getHelpText())
                        .setChatStyle(new ChatStyle().setColor(EnumChatFormatting.YELLOW)));
                if(iter.hasNext()) {
                    message.appendText("\n");
                }
            ...
            // There are no commands to display a help message for.
            message.appendSibling(new ChatComponentTranslation(
                    "quickplay.commands.quickplay.premium.noCommands")
