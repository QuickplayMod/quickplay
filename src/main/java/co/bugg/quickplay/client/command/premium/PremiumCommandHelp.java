        ...
        if(args.length < 2) {
            ...
            if(filteredList.size() > 0) {
                final IChatComponent chatComponent = new ChatComponentText(filteredList.get(0).getHelpText() + "\n")
                        ...
                        .appendSibling(new ChatComponentText("/" + parent.getParent().getName() + " " + parent.getName() + " " + filteredList.get(0).getName() + " " + filteredList.get(0).getUsage()))
    ...
    public void sendBaseHelpMessage() {
        ...
        if(parent.premiumCommands.size() > 0) {
            for(ListIterator<IPremiumCommand> iter = parent.premiumCommands.listIterator(); iter.hasNext();) {
                ...
                message.appendSibling(new ChatComponentText("/" + parent.getParent().getName() + " " + parent.getName() + " " + premiumCommand.getName()))
