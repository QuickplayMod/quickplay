    ...
    public void initGui() {
        ...
        if(appData != null) {
            ...
            if (currentState == State.ADROLL) {
                ...
                componentList.add(new QuickplayGuiString(storeUrl, currentId++, width / 2, (int) (height / 2 + adTextureSize / 2 * adScale), width, 20, I18n.format("quickplay.premium.ingameReward.adroll.clickToVisit"), true, false));
                ...
                for (DailyRewardOption option : appData.rewards) {
                    componentList.add(new QuickplayGuiButton(option, currentId++, (int) ((width / 2 - (cardWidth + 36) * cardScale / 2) + ((currentCard - cardCount / 2) * (cardWidth * cardScale + cardMargins))) + xOffset, (int) (height / 2 - cardHeight * cardScale / 2), cardWidth, cardHeight, null, option.getTexture(), 0, 0, cardScale, false));
                    currentCard++;
                }
                ...
                componentList.add(new QuickplayGuiButton("https://rewards.hypixel.net/claim-reward/shared/" + appData.id, currentId++, width / 2 - sharedWidth / 2, (int) (height / 2 + cardHeight / 2 * cardScale) + 5, sharedWidth, sharedHeight, I18n.format("quickplay.premium.ingameReward.claimed.share"), false));
            }
            ...
            if (appData.id != null && (currentState == State.MENU || currentState == State.ADROLL || currentState == State.ERROR)) {
                ...
                componentList.add(new QuickplayGuiButton("https://rewards.hypixel.net/claim-reward/" + appData.id, currentId++, width - linkButtonWidth - linkButtonMargins, height - linkButtonHeight - linkButtonMargins, linkButtonWidth, linkButtonHeight, I18n.format("quickplay.premium.ingameReward.openLink"), false));
            }
        }
    }
